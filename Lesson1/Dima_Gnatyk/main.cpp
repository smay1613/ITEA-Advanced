...
template <class T>
std::vector<T> combineVectors(const std::vector<T> &data_vector_1, const std::vector<T> &data_vector_2){
    if(data_vector_1.empty())
    {
        return data_vector_2;
    }

    if(data_vector_2.empty())
    {
        return data_vector_1;
    } 
    ...
    union_vector.insert(union_vector.cend(), data_vector_2.cbegin(), data_vector_2.end());

    return union_vector;
}

template <class T>
std::list<T> magicNumber(const T &number, const T &pow){
    std::list<T> result {};
...
template<class T>
void reverse(std::forward_list<T>& data){
    if(data.empty()){
    ...
    data.clear();
    data.splice_after(data.before_begin, result);
}
