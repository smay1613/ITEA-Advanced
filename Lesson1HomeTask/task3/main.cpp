...
{
    std::vector<typename std::forward_list<T>::iterator> f_list_iterators;

    for (typename std::forward_list<T>::iterator it = f_list.begin(); it != f_list.end(); it++)
    ...
    {
        std::swap( *f_list_iterators[i],  *f_list_iterators[f_list_iterators_size -1 - i]);
        //std::cout << "Step " << i << " List: ";
        //print_list (f_list);
        //std::cout << std::endl;
    }
